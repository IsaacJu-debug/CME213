...
randMT::randMT() { seed(1U); }

randMT::randMT(uint32 seed_) { seed(seed_); }
...
inline double randMT::rand(const double& n) { return rand() * n; }

inline double randMT::randExc() {
  return double(random()) * 2.3283064365386963e-10;
}

inline double randMT::randExc(const double& n) { return randExc() * n; }
