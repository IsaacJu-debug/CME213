...
TEST(CUDA, add_matrices)
{
    ...
    vector<int> h_c(n * n);
    checkCudaErrors(cudaMemcpy(&h_c[0], d_c, sizeof(int) * n * n, cudaMemcpyDeviceToHost));

    // sanity check to ensure that h_c get the proper results
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < n; ++ j)
        {
            ASSERT_EQ(h_c[n * i + j], i - j) << "Wrong results!";
        }
    }
}
